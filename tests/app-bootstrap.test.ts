...
import { getRouter } from '../src/services/runtime';
...
function seedAgents(): void {
  ...
  fs.writeFileSync(path.join(tmp, 'agents', 'agentA', 'config.json'), JSON.stringify({
    ...
    plugins: [{ name: 'builtin', tools: ['read', 'math'] }],
  }), 'utf-8');
}
...
describe('bootstrap（L5 冒烟）', () => {
  ...
  it('已有 admin 时不重复创建 agent_chat_dev（非首次）', async () => {
    ...
    const result = await bootstrap({ enableWebUI: false });
    try {
      expect(result.registry.has('agent_chat_dev')).toBe(false);
