...
import { run } from '../src/core/loop';
import { createContext, pushSteer } from '../src/core/context';
import { hashDialogId } from '../src/core/hash';
import type { CurrentContext } from '../src/core/context';
import type { LLMProvider, LLMRequest, LLMRequestMessage, LLMResponse, Tool } from '../src/core/types';
...
describe('run —— usage 与事件', () => {
  ...
  it('chat.end：致命兜底路径也发射（事件流始终闭合）', async () => {
    ...
    const boomMsg = { role: 'user' as const, get content() { throw new Error('boom'); } };
    const result = await run(createContext({
...
describe('run —— 生命周期钩子', () => {
  ...
  it('turnEndHook：观察本轮结果与本轮产出', async () => {
    ...
    await run(createContext({
      llm, systemPrompt: 's', history: [], currentMessage: userMsg, tools: new Map(),
      turnEndHook: [(ctx, outcome, loopMessages) => { seen.push({ outcome, produced: loopMessages.length }); }],
    }));
  ...
  it('toolExecutionEndHook：观察工具结果', async () => {
    ...
    await run(createContext({
      llm, systemPrompt: 's', history: [], currentMessage: userMsg,
      tools: new Map([['add', tool]]),
      toolExecutionEndHook: [(o) => { outcomes.push(o); }],
    }));
  ...
  it('fallbackHook：LLM 调用失败时触发，run 正常返回', async () => {
    ...
    const result = await run(createContext({
      llm, systemPrompt: 's', history: [], currentMessage: userMsg, tools: new Map(),
      fallbackHook: [(_ctx, err) => { calls.push(err); }],
    }));
  ...
  it('流式 finishReason=error：触发 fallbackHook + 产出 error 消息（B1）', async () => {
    ...
    expect(result.interrupted).toBe(false);
    expect(result.messages.some(m => m.role === 'error')).toBe(true);
    // 事件流：含 error 事件，且 chat.end 闭合
    expect(events).toContain('chat.message.error');
    expect(events[0]).toBe('chat.start');
    expect(events[events.length - 1]).toBe('chat.end');
  });
  ...
  it('runStartHook：可修改 ctx（如注入运行时字段）', async () => {
    ...
    await run(createContext({
      llm, systemPrompt: 's', history: [], currentMessage: userMsg, tools: new Map(),
      runStartHook: [(ctx) => { (ctx as any).customField = 'injected'; }],
    }));
