// ============================================================
// ac-edit-core/src/fuzzy-match.ts —— old_string 模糊匹配（src edit 原样继承）
//
 ...
 */
export function normalizeForFuzzyMatch(text: string, trimLeading = false): string {
  return text
    // Unicode 正规化
    .normalize('NFKC')
    // 按行处理：去除空白
...
export function fuzzyFindText(content: string, oldText: string): FuzzyMatchResult {
  ...
  if (fuzzyIndex !== -1) {
    ...
    // 内容中的位置，必须映射回原始 content 才能用于切片（src P0 修复）。
...
/** 统计 oldText 在 searchIn 中的出现次数（useFuzzy 时按归一化形态计数） */
