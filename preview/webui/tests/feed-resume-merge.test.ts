...
describe('mergeResumeSnapshot：切回运行中的 Agent（即时合并路径）', () => {
  ...
  it('steps 含进行中步骤（后端现行序列化）→ 不新建第二个流式占位', () => {
    ...
    feed.ingestFrame('llm/delta', [{ model: 'm' }, { reasoning: '' }, { agent: A, conversationId: A, sender: 'user' }]);
    feed.ingestFrame('llm/delta', [{ model: 'm' }, { reasoning: '思考' }, { agent: A, conversationId: A, sender: 'user' }]);
    feed.ingestFrame('llm/delta', [{ model: 'm' }, { delta: '测' }, { agent: A, conversationId: A, sender: 'user' }]);
    ...
    // ③ 直播继续
    feed.ingestFrame('llm/delta', [{ model: 'm' }, { delta: '试' }, { agent: A, conversationId: A, sender: 'user' }]);
  ...
  it('快照内容落后于直播（subscribe 往返竞态）→ 不回卷已渗出的正文', () => {
    ...
    feed.append(id, { id: 'u1', role: 'agent', content: '你好', timestamp: Date.now(), agent_id: 'user' });
    feed.ingestFrame('loop/step-started', [A, 0, [], { conversationId: A, sender: 'user' }]);
    feed.ingestFrame('llm/delta', [{ model: 'm' }, { delta: '测试完成' }, { agent: A, conversationId: A, sender: 'user' }]);
    ...
    feed.ingestFrame('llm/delta', [{ model: 'm' }, { delta: '！' }, { agent: A, conversationId: A, sender: 'user' }]);
  ...
  it('空分区（页面刷新，历史未到）→ resume 挂起，历史首屏后合并出唯一载体', async () => {
    ...
    // 历史首屏（当前轮 user 消息已落盘；assistant 未 checkpoint）——Port B RPC
