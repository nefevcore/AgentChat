...
//
// Port B（P1）：preview 无 /api/plugins/*-schemas 面——schema 为空时
// 走【字段直连】合成：provider 选项 = schema ∪ 池内条目 provider；
// 字段集 = 每类基线字段 ∪ 既有条目里观测到的键（类型按值推断）。
// 有真 schema（后续补面）时合成结果自动让位。
// ============================================================
...
/** 池内各 provider 观测到的额外字段（基线之外，类型按值推断） */
function inferExtraFields(kind: 'llm' | 'search', pools: Record<string, PoolEntry>): Map<string, FieldMeta[]> {
  const byProvider = new Map<string, Map<string, FieldMeta>>();
  const baseKeys = new Set(BASE_FIELDS[kind].map((f) => f.key));
