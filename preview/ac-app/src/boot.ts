...
const previewDir = path.resolve(fileURLToPath(new URL('../../', import.meta.url)));
console.log(`[boot] 装配根（chdir → cordis.yml 所在目录）: ${previewDir}`);
...
if (process.env.AGENTCHAT_SUPERVISED !== '1') {
  const lockFile = runtimeLockPath(previewDir);
  try {
    ...
    console.error(
      `[boot] 检测到另一实例正在运行（${lockFile} 已被锁定）。` +
        '同数据根双写者会互相覆盖（config/singles/会话文件）；确认另一实例（preview:boot 直启或 preview:supervised）后重试。',
    );
