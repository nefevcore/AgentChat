...
// 用法：AGENTCHAT_DATA_ROOT=<目标根> pnpm exec tsx scripts/migrate-workspace.ts [源目录=../workspace/preview 相对 preview/]
// ============================================================
...
// 脚本位于 preview/scripts/ → DST 缺省 = preview/data；AGENTCHAT_DATA_ROOT
// 给定则用之（M18 数据根约定：迁移写到实际数据根）。
// 源缺省 = 仓库根 workspace/preview
const SCRIPTS_DIR = fileURLToPath(new URL('.', import.meta.url)); // …/preview/scripts/
const PREVIEW_DIR = path.resolve(SCRIPTS_DIR, '..');
const DST = process.env.AGENTCHAT_DATA_ROOT
  ? path.resolve(process.env.AGENTCHAT_DATA_ROOT)
  : path.join(PREVIEW_DIR, 'data');
const srcArg = args.find((a) => !a.startsWith('--'));
const SRC = path.resolve(PREVIEW_DIR, srcArg ?? '../workspace/preview');
