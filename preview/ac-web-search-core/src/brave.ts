// ============================================================
// ac-web-search-core/src/brave.ts —— Brave Search Provider（src 原样平移）
// ============================================================
...
export function createBraveProvider(): SearchProvider {
  return {
    ...
    validateConfig(cfg: ProviderConfig): void {
      if (!cfg.apiKey) {
        throw new Error('未配置 Brave Search API 密钥。可前往 https://brave.com/search/api/ 免费获取（每月 2000 次免费额度）。');
      }
    },
    ...
    async search(params: SearchParams, cfg: ProviderConfig): Promise<SearchResponse> {
      ...
      const data = (await response.json()) as {
        ...
        discussions?: Array<{ snippet?: string }>;
        infobox?: Array<{ description?: string }>;
      };
      ...
      const answer = data.discussions?.[0]?.snippet ?? data.infobox?.[0]?.description ?? null;
