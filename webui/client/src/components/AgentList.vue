...
async function openAddDialog() {
  ...
  if (Object.keys(llmPools.value).length === 0) {
    try {
      ...
      if (r.ok) { const data = await r.json(); llmPools.value = data.llmProviders ?? {}; }
    } catch { /* ignore */ }
  }
}
...
const unifiedList = computed<UnifiedItem[]>(() => {
  ...
  for (const a of agentStore.agents) {
    items.push({ type: 'agent', id: a.id, name: a.name || a.id, lastActivity: a.lastActivity ?? 0, agent: a });
  }
  for (const g of props.groups) {
    items.push({ type: 'group', id: g.group_id, name: g.name, lastActivity: 0, group: g });
  }
...
const filteredItems = computed(() => {
  ...
  return unifiedList.value.filter(item => item.name.toLowerCase().includes(q));
});
...
function selectAgent(id: string) {
  ...
  if (chatStore.messages.length === 0) chatStore.loadHistory('user', id);
...
function formatLastMessage(lm: AgentInfo['lastMessage']): string {
  if (!lm?.content) return '';
  const prefix = lm.agent_id === 'user' ? '你: ' : '';
...
async function createAgent() {
  ...
  try {
    ...
    if (selectedLlmPool.value) {
      ...
      if (pool) {
        const pd: Record<string, any> = { $ref: selectedLlmPool.value };
        for (const [k, v] of Object.entries(pool)) {
          if (k !== '$ref' && k !== '$comment' && !k.startsWith('$')) pd[k] = v;
        }
        body.llm = pd;
      }
    }
    ...
    showAddDialog.value = false; newAgentId.value = ''; newAgentName.value = ''; addError.value = '';
...
function getGroupAvatars(g: GroupInfo): PAv[] {
  return g.participants.slice(0, 9).map(id => ({ avatar: agentStore.getAgentAvatar(id), name: agentStore.getAgentName(id) }));
}
  ...
  <div class="agent-list">
    ...
    <div class="list-scroll">
      <div v-for="item in filteredItems" :key="item.type + '-' + item.id" class="list-item"
        ...
        <!-- Agent 头像 -->
        <div v-if="item.type === 'agent'" class="item-avatar-wrap">
          <div class="item-avatar">
            <img v-if="item.agent?.avatar" :src="item.agent.avatar" :alt="item.name" />
            <div v-else class="avatar-placeholder">{{ item.name.charAt(0).toUpperCase() }}</div>
          </div>
          <span v-if="unreadAgents.has(item.id)" class="unread-dot" />
        </div>

        <!-- 群组合并头像 -->
        <div v-else-if="item.group" class="group-avatar"
          :style="{ display: 'grid', gridTemplateColumns: `repeat(${gridLayout(getGroupAvatars(item.group).length).cols}, 1fr)`, gridTemplateRows: `repeat(${gridLayout(getGroupAvatars(item.group).length).rows}, 1fr)` }">
          <template v-for="(p, idx) in getGroupAvatars(item.group)" :key="idx">
            <img v-if="p.avatar" :src="p.avatar" :alt="p.name" class="group-avatar-cell" />
            <span v-else class="group-avatar-cell group-avatar-placeholder">{{ p.name.charAt(0).toUpperCase() }}</span>
          </template>
          <svg v-if="getGroupAvatars(item.group).length === 0" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /></svg>
        </div>

        <div class="item-info">
          <div class="item-name">{{ item.name }}</div>
          <div v-if="item.type === 'agent' && item.agent" class="item-last-msg">{{ formatLastMessage(item.agent.lastMessage) }}</div>
          <div v-else-if="item.type === 'group' && item.group" class="item-last-msg">{{ item.group.participants.length }} 个参与者</div>
        </div>
      </div>

      <div v-if="filteredItems.length === 0 && unifiedList.length > 0" class="empty">无匹配的会话</div>
      <div v-else-if="unifiedList.length === 0" class="empty">暂无会话</div>
    </div>
    ...
    <Transition name="modal">
      <div v-if="showAddDialog" class="dialog-overlay" @mousedown.self="showAddDialog = false">
        <div class="dialog-panel" @click.stop>
          <h4>新增 Agent</h4>
          <div class="form-group"><label>Agent ID <span class="optional-hint">（可选，留空自动生成）</span></label><input v-model="newAgentId" type="text" placeholder="如 my_agent，留空则自动生成 UUID" @keyup.enter="createAgent" /></div>
          <div class="form-group"><label>显示名称</label><input v-model="newAgentName" type="text" placeholder="如 我的助手" @keyup.enter="createAgent" /></div>
          <div class="form-group"><label>模型</label><select v-model="selectedLlmPool"><option value="">默认（全局配置）</option><option v-for="(entry, name) in llmPools" :key="name" :value="name">{{ name }}{{ (entry as any).model && (entry as any).model !== name ? ' · ' + (entry as any).model : '' }}</option></select></div>
          <p v-if="!selectedLlmPool" class="default-hint">将使用全局默认模型配置</p>
          <div v-if="addError" class="error-text">{{ addError }}</div>
          <div class="dialog-actions"><button class="btn-cancel" @click="showAddDialog = false">取消</button><button class="btn-save" @click="createAgent">创建</button></div>
        </div>
      </div>
    </Transition>
  </div>
...
<style scoped>
.agent-list { flex: 1; min-width: 0; background: var(--color-bg-surface); border-right: 1px solid var(--color-border-secondary); display: flex; flex-direction: column; z-index: 210; transition: transform 0.25s ease; }
...
.search-input { width: 100%; padding: 5px 8px 5px 28px; border: 1px solid var(--color-border-secondary, #ddd); border-radius: 6px; background: var(--color-bg-page, #fff); color: var(--color-text-primary, #2c3e50); font-size: 13px; outline: none; transition: border-color 0.15s; }
...
.avatar-placeholder { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; background: var(--color-primary-light, rgba(79,70,229,0.12)); color: var(--color-primary, #4f46e5); font-size: 15px; font-weight: 600; }
...
.group-avatar { width: 40px; height: 40px; border-radius: 6px; display: flex; align-items: center; justify-content: center; background: var(--color-primary-light, rgba(79,70,229,0.12)); color: var(--color-primary, #4f46e5); flex-shrink: 0; gap: 1px; padding: 2px; box-sizing: border-box; overflow: hidden; }
...
.dialog-panel { background: var(--color-bg-page, #fff); border-radius: 10px; padding: 20px 24px; width: 360px; max-width: 90vw; max-height: 85vh; overflow-y: auto; box-shadow: 0 8px 32px rgba(0,0,0,0.15); }
...
@media (max-width: 768px) {
  .agent-list { position: fixed; top: 0; left: 0; bottom: 0; width: min(280px, 80vw); transform: translateX(-100%); box-shadow: 2px 0 16px rgba(0, 0, 0, 0.15); }
  .agent-list.sidebar-mobile-visible { transform: translateX(0); }
