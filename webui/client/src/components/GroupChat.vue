...
async function saveGroupName() {
  ...
  renameSaved.value = false;
  try {
    const resp = await fetch(`/api/rooms/${encodeURIComponent(props.group.room_id)}`, {
      method: 'PATCH',
...
async function confirmDelete() {
  ...
  deleteError.value = '';
  try {
    const resp = await fetch(`/api/rooms/${encodeURIComponent(props.group.room_id)}`, { method: 'DELETE' });
    const data = await resp.json();
...
async function loadGroupHistory() {
  if (!props.group) return;
  try {
    const resp = await fetch(`/api/rooms/${props.group.room_id}/history?limit=50`);
    if (!resp.ok) return;
    const data = await resp.json();
