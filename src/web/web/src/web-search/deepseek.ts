// ============================================================
// DeepSeek 联网搜索 Provider
// 移植自 DSH @deepseek-ai/dsh-web-search-deepseek：DeepSeek 无专用检索端点，
// 而是调用其 Anthropic 兼容 Messages API（POST {baseURL}/messages）并启用
// 原生 web_search_20250305 服务端工具，从结构化 web_search_tool_result 块
// 解析结果。注意：一次搜索 = 一次完整模型回合（计 token，比纯检索端点慢）。
//
...
//
// 内容来源（实测 DeepSeek 响应，2026-08）：web_search_result 项只有
// title/url/page_age + encrypted_content（密文，仅模型服务端可解）；
// text 块不带 citations（DSH 的 cited_text 回填路径拿不到任何东西）。
// 因此让模型在搜索后按 SOURCE:/SUMMARY: 结构逐源摘要（它能读解密后的
// 页面内容），并输出 ANSWER: 综合段——按 URL 解析回填每条结果的 content。
// citations 回填逻辑保留为首选（若日后 DeepSeek 补齐该字段则优先采用）。
// ============================================================

import type { SearchProvider, SearchParams, SearchResponse, ProviderConfig, SearchResult } from './types';
 ...
 */
export function mapAnthropicResponse(data: any, maxResults: number): DeepSeekSource[] {
  ...
  const textBlocks = blocks.filter((b) => b.type === 'text' && typeof b.text === 'string').map((b) => b.text as string);
...
export function createDeepSeekProvider(): SearchProvider {
  return {
    id: 'deepseek',
    label: 'DeepSeek 联网搜索',
    description: 'DeepSeek 官方联网搜索（Anthropic 兼容 Messages API + 原生 web_search 服务端工具）。与 DeepSeek 模型共用同一 API Key；每次搜索消耗一次模型调用 token，返回逐源摘要与综合答案',

    validateConfig(cfg: ProviderConfig): void {
      if (!cfg.apiKey) {
        throw new Error(
          '未配置 DeepSeek API 密钥。请在池条目的 API 密钥字段中设置' +
          '（与 DeepSeek 模型共用同一 Key），或设置环境变量 DEEPSEEK_API_KEY。' +
    ...
    async search(params: SearchParams, cfg: ProviderConfig): Promise<SearchResponse> {
      ...
      const textBlocks = (data?.content ?? [])
        ...
        .map((b: AnthropicBlock) => b.text as string);
      ...
      const results: SearchResult[] = sources.map((s, i) => ({
        title: s.title,
        url: s.url,
        content: cap(s.snippet
          ? (s.publishedAt ? `[${s.publishedAt}] ${s.snippet}` : s.snippet)
          : (s.publishedAt ? `发布时间：${s.publishedAt}` : '')),
