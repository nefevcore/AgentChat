...
function fail(message: string): void {
  ...
  postToParent({ source: 'agentchat-ui-iframe-host', plugin: pluginName, kind: 'error', error: message });
}
...
async function boot(): Promise<void> {
  ...
  try {
    ...
    postToParent({ source: 'agentchat-ui-iframe-host', plugin: pluginName, kind: 'ready', ok: true });
  } catch (err) {
