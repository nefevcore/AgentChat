...
function lastStreaming(msgs: ChatMessage[], role?: 'agent' | 'tool'): ChatMessage | null {
  ...
  function loadMoreHistory() {
    ...
    // 发送前先累加 offset（防连续滚动重复请求同一页）；后端按轮次(user 链)分页
    // 累加步长 = 请求的 limit（HISTORY_PAGE_SIZE 轮），onHistory 收到响应后按实际轮数校准
...
function onHistory(data: any) {
    ...
    // 后端按轮次(user 链)分页：本页返回的 viewer 消息数 >= 页大小(5 轮) → 可能还有更早
    ...
    // 这里按本页实际返回的轮数（viewer 消息数）校正为"已加载轮数"，使下次请求从正确位置继续。
  ...
  const HANDLERS: Record<string, (d: any) => void> = {
    ...
    'chat.message.error':   d => { if (!isForCurrentUser(d)) return; onMessageError(eventAgentId(d), d); },
    ...
    'chat.toolcall.start':  d => { if (!isForCurrentUser(d)) return; onToolcallStart(eventAgentId(d), d); },
    'chat.toolcall.update': d => { if (!isForCurrentUser(d)) return; markActive(); },
    'chat.toolcall.end':    d => { if (!isForCurrentUser(d)) return; markActive(); },
