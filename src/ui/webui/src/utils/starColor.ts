...
/** 获取 Agent 星色标签（调试/工具提示用） */
export function starLabel(agentId: string): string {
  if (!agentId || agentId === 'user') return USER_STAR.label;
  return STAR_PALETTE[hashAgentId(agentId)]!.label;
}

/** 读取当前生效主题（DOM 标记，非响应式；供一次性调用） */
export function detectTheme(): ThemeMode {
  const el = document.documentElement;
  if (el.dataset.theme === 'aurora') return 'aurora';
  if (el.classList.contains('light')) return 'aurora';
  if (el.classList.contains('dark')) return 'nebula';
  return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'nebula' : 'aurora';
}

/** 主题模式 → 深空色板（保留给 UI 库内部使用） */
export function paletteOf(theme: ThemeMode) {
  return STAR_PALETTE;
}
