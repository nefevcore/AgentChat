...
//   1. 标题栏（文本「运行跟踪」）+ 移动端关闭
//   2. 运行总览（树节点）→ 统计叶节点 + 「运行矩阵」入口叶节点（打开主区大画布）
...
import { interruptRun } from '../core/api/endpoints/runs';
import type { RunsRunningEntry } from '../core/api/endpoints/runs';
...
async function jumpTo(r: RunsRunningEntry) {
  ...
  if (t.kind === 'single') {
    ...
    if (a?.hasActiveSession) wsStore.send(WS_SEND.chatSubscribe, { to: t.id });
  }
...
<template>
  <div class="runs-panel">
    ...
    <div class="tree-scroll">
      <!-- 2. 运行总览：节点本身即矩阵入口（点击打开/关闭主区运行矩阵） -->
      <div class="tree-node action" :class="{ active: ui.trackingViewVisible }"
        ...
        <span class="node-name">运行总览</span>
        <span v-if="running.length > 0" class="node-badge">{{ running.length }}</span>
        <span class="node-action-icon"><Icon :name="ui.trackingViewVisible ? 'x' : 'chevron-right'" :size="13" /></span>
      </div>
...
.node-action-icon{display:flex;align-items:center;justify-content:center;color:var(--color-text-tertiary,#a8abb2);flex-shrink:0}
/* 总览节点：可执行动作（打开矩阵）→ 主色强调 + 选中态 */
.tree-node.action .node-icon.kind-overview{color:var(--color-primary,#6366f1)}
.tree-node.action .node-name{color:var(--color-primary,#6366f1)}
.tree-node.action .node-action-icon{color:var(--color-primary,#6366f1)}
.tree-node.action.active{background:var(--role-selected-bg,#e6eaff);border-color:transparent}
