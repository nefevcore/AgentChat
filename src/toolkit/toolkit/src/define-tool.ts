...
export interface DefineToolInput {
  ...
  ns?: string;
  /** 能力标签要求（受控词汇表 base/dev/admin/conductor）：
   *  · requires 非空且全部命中 Agent 能力标签 → 默认启用（base 隐式）
   *  · requires 未命中 → 权限门禁（tools.include 也无法绕过）
   *  · requires 缺省 → 默认关闭，只能 tools.include 显式启用 */
  requires?: string[];
  /** 描述（给 LLM 看） */
  description: string;
  /** JSON Schema 参数 */
