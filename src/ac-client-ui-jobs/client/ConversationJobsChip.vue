// ============================================================
// client/ConversationJobsChip.vue —— 会话头任务清单入口
//（会话区重构自 ac-client-ui-conversation 迁域归位：jobs 域组件住 jobs
//  行，经 conversation:header-widget 席位贡献 order 10——数据与组件
//  同域（ctx.jobBoard），conversation 只开席不持域资产。）
//
// 各会话头（ConversationView header-actions）的「后台任务 / 子Agent 调用」
// 双清单入口：按发起会话键（conversationId）过滤 jobs 域投影（ctx.jobBoard）——
// 本会话 run 里启动的 bash 后台与 subagent 委派（对桶键 / singles sid /
// 群 gid 同词表）。数据与侧边栏运行跟踪面板同源（单一域投影，
// job/started·settled 帧驱动刷新）；本会话无任务时不渲染（零占位）。
// 弹层形态对齐会话头 Token 仪表（token-panel：头部下挂 + 点外关闭）。
// ============================================================

<script setup lang="ts">
import { computed, onMounted, onUnmounted, ref, watch, toRef } from 'vue';
import { Icon, formatDurationMs } from '@agentchat/webui-kit';
...
const scoped = computed(() => jobsForConversation(jobBoard?.jobs.value ?? [], conversationId.value));
