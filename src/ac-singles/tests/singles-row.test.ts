// ============================================================
// ac-singles/tests/singles-row.test.ts —— 行包双半边宿主面验收
...
describe('S3-1b · ac-singles 宿主半边（boot graph 声明）', () => {
  it('行装载 → boot graph 含 singles；卸载 → 级联回缩 + 变更帧', async () => {
    ...
    expect(ctx.webui.listBootGraph().map((g) => g.name)).toContain('singles');
    await fiber.dispose();
    expect(ctx.webui.listBootGraph().map((g) => g.name)).not.toContain('singles');
    expect(changed.filter((n) => n === 'singles').length).toBeGreaterThanOrEqual(2);
    off();
  });
});
