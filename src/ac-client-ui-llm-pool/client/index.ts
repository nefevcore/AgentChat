...
// 半边（M28 P2 §5.2：PoolManager 随域成行）
//
// 贡献面：settings:section 选举席（meta.section = 'llmPools' /
// 'searchPools'）——settings 壳按 selectedNode 选举渲染；池更新/
// 默认同步/定向落盘编排随 Host 走（settings 共享 store 经跨包 import，
// domain→base 合法）。
// ============================================================
  ...
  apply(ctx: ClientContext) {
    ...
    ctx.slots.inject('settings:section', () =>
      ctx.slots.register('settings:section', {
        ...
        meta: { section: 'llmPools' },
      }),
    );
    // 搜索引擎节（search 池）
    ctx.slots.inject('settings:section', () =>
      ctx.slots.register('settings:section', {
        id: 'webui-domain-llm-pool.search',
        component: SearchPoolsHostAsync,
        meta: { section: 'searchPools' },
      }),
    );
  },
});
