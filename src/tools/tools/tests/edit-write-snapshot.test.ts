...
import { makeWriteTool } from '@agentchat/fs';
import { makeEditTool } from '@agentchat/edit';
import { recordSnapshot, clearSnapshot, verifySnapshot, verifySnapshotDetailed, getSnapshot } from '@agentchat/edit';
import { computeFileHash } from '@agentchat/edit';
...
describe('JSON edit 后同步 hashline 快照（连续编辑不被误拒）', () => {
  ...
  it('第一次 edit 改变行数后，第二次裸行号编辑仍命中目标行', async () => {
    ...
    expect(JSON.parse(r1 as string).status).toBe('success');
    ...
    expect(JSON.parse(r2 as string).status).toBe('success');
...
describe('verifySnapshotDetailed 失败原因诊断', () => {
  ...
  it('快照与请求 TAG 不一致（快照过期/写改写未同步）→ snapshot-mismatch', () => {
    ...
    if (!r.ok) {
      ...
      expect((r as any).snapshotTag).toBe(computeFileHash(oldContent));
    }
  });
