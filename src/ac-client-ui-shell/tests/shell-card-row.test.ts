// ============================================================
// ac-client-ui-shell/tests/shell-card-row.test.ts —— 工具卡行验收
...
describe('M28 P2 · ac-client-ui-shell 宿主半边（boot graph 声明）', () => {
  it('行装载 → boot graph 含 ui-shell（phase=domain）；卸载 → 级联回收 + 变更帧', async () => {
    ...
    expect(graph.map((g) => g.name)).toContain('ui-shell');
    const def = graph.find((g) => g.name === 'ui-shell')!;
    expect(def.phase).toBe('domain');
    expect(def.entry).toMatch(/ac-client-ui-shell[\\/]client[\\/]index\.ts$/);
    await fiber.dispose();
    expect(ctx.webui.listBootGraph().map((g) => g.name)).not.toContain('ui-shell');
    expect(changed.filter((n) => n === 'ui-shell').length).toBeGreaterThanOrEqual(2);
    off();
  });
});
