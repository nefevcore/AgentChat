...
//
// inject: bootstrap, agentLoop —— 引擎由 agent-loop 插件行提供。
// ============================================================
...
export const inject = ['bootstrap', 'agentLoop'];
...
export function apply(ctx: Context) {
  ...
  ctx.logger('subagent').info('SubAgentManager 由 subagent 插件行持有（ctx.subagent.manager）');
}
