// ============================================================
// @agentchat/agent-skill/src/skills.ts —— 技能发现与展示
//
// 自 @agentchat/agent-prompt 拆出：解析 <agentDir>/skills/*/SKILL.md
// frontmatter → 渲染 <available_skills> 区块 → discovered_skills 钩子注入。
//
// 技能路径：<agentsDir>/<agentDirName>/skills/<dirName>/SKILL.md
//
// 依赖方向：仅依赖 Node fs/path + agent-config/toolkit/tools 类型。
// ============================================================
...
// ============================================================
// 独立注入钩子（discovered_skills）
// ============================================================

/**
 * runStart：发现并注入技能清单到 ctx.systemPrompt 末尾。
 * 独立于 build-system-prompt（该钩子只装配角色/环境/指引/存储，不含技能）。
 * 执行顺序由插件 runStart 声明控制（agent-skill.discovered_skills 置于 open-mcp 前）。
 */
export function makeInjectSkillsHook(config: AgentConfig, services: ToolContext): RunStartHook {
