// ============================================================
// src/ac-client-ui-runview/tests/runview-row.test.ts —— 前端行验收
//
// S3/D19：宿主半边（boot graph 声明）+ 卸载级联（声明回收 → 前端
// 消费面消失的服务端证据）。M27.1 改名入 ac-client-ui-* 全族
//（boot graph 键 = 派生名 ui-runview）。bootTree 全树 HTTP 面见
// webui/tests/boot-graph-http.test.ts。
// ============================================================
...
describe('M27.1 · ac-client-ui-runview 宿主半边（boot graph 声明）', () => {
  it('行装载 → ctx.webui boot graph 含 ui-runview 条目（entry 为绝对路径）', async () => {
    const ctx = await boot();
    const fiber = await loadRow(ctx);
    const graph = ctx.webui.listBootGraph();
    expect(graph.map((g) => g.name)).toContain('ui-runview');
    const def = graph.find((g) => g.name === 'ui-runview')!;
    expect(def.platform).toBe('web');
    expect(def.entry).toMatch(/ac-client-ui-runview[\\/]client[\\/]index\.ts$/);
    await fiber.dispose();
  });
  ...
  it('可摘除性（D19）：行卸载 → 声明级联回收 → boot graph 收缩（前端消费面消失的服务端证据）', async () => {
    const ctx = await boot();
    const fiber = await loadRow(ctx);
    expect(ctx.webui.listBootGraph().map((g) => g.name)).toContain('ui-runview');
    await fiber.dispose();
    expect(ctx.webui.listBootGraph().map((g) => g.name)).not.toContain('ui-runview');
  });
});
