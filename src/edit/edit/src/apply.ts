...
// 行级编辑（applyLineEdits / resolveSnapshotHash / 行哈希验证）已删除。
//
// 依赖方向：types / fuzzy-match。
// ============================================================

import { fuzzyFindText, normalizeForFuzzyMatch, countOccurrences } from './fuzzy-match';
import type { ReplaceEdit, AppliedEditsResult, EditPosition, FuzzyMatchResult } from './types';
...
export function applyEditsToNormalizedContent(
  ...
  for (const edit of edits) {
    ...
    if (edit.oldText.length === 0) {
      throw new Error(
        `编辑失败：old_string 不能为空。请提供要替换的精确文本。`
      );
    }
