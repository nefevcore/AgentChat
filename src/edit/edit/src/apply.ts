  ...
  for (const edit of edits) {
    ...
    if (!matchResult.found) {
      throw new Error(
        ...
        `oldText 必须与文件内容匹配（引号/空白差异可自动归一化）。` +
        `\n未找到的文本："""\n${edit.oldText.slice(0, 300)}"""` +
        (edit.oldText.length > 300 ? '\n...（已截断）' : '') +
        `\n恢复建议：① 先用 read 查看该文件当前内容，从输出复制精确原文；② oldText 尽量短而独特（避免空白/换行差异）；③ 大文件可用 op+pos 行级定位。`
      );
    }
    ...
    if (occurrences > 1) {
      throw new Error(
        ...
        `\n重复文本："""\n${edit.oldText.slice(0, 300)}"""` +
        (edit.oldText.length > 300 ? '\n...（已截断）' : '') +
        `\n恢复建议：① 在 oldText 中多包含前后几行，让目标位置独一无二；② 或用 op+pos 行级定位（先用 read 拿行号）。`
      );
    }
