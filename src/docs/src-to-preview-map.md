...
| agent-presets | **ac-agent-presets** ✅（预设模式注册中心）+ **ac-agent-presets-builtin** ✅（内置数据行，2026-09 拆分）| 注册中心：数据行经 ctx.agentPresets.register 注入预设定义（register 即归属）→ 物化（preset 标志：名册过滤/send_agent 拒收/管理面写口拦截）+ 默认池模型解析 + config/changed 热更新——插件注入自有模式同一注册面；内置 __standard__/__dsh_minimal__ 住数据行；无记忆语义 = hooks 软停用 memory/skill/datetime（dsh-minimal 另停 system-prompt）；allowlist 语义由行组合承担（ADR-4 缩水） |
