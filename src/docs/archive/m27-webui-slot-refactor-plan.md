...
> 轨迹，勿按本文施工）：hostLedger（M27.2-1 退役——席位由 owning 件
> 自声明）、服务端 BUILTIN_SLOTS 白名单（M30 D7 随 ctx.uiExtensions
> 退役）、keyed 选举席/数据贡献的注释级约定（M30 D1/D2 升 elect/data
> 词汇轴）、`tracking:dock-widget`（M30 D5 改名 conversation:dock-widget）。
> 现行事实源：`../ui-rows-and-slots.md`、`../webui-slot-tree.md`、
> `../m30-slot-semantics-refinement-plan.md`。
