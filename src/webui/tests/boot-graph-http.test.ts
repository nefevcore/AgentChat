...
describe('S3 · /api/ui/boot-graph（bootTree 真树 HTTP 面）', () => {
  it('全树 boot → graph 含 runview 条目（服务面 + HTTP 路由）', { timeout: 30_000 }, async () => {
    ...
    // M27.2：基础件出包（phase:'base'——封印前批次；
    // theme/renderer/tool/sidebar/conversation/settings 已出包）
    for (const name of ['ui-theme', 'ui-renderer', 'ui-tool', 'ui-sidebar', 'ui-conversation', 'ui-settings']) {
