...
// 本身；浏览器端只会更慢）。
//
// 口径说明（2026-01 前端性能改造后）：真实管线已改为「按需装配」——
// KaTeX（katex + texmath ≈ 530KB）与冷门 hljs 语言仅在首次遇到时才
// 加载，加载期间该部分回落原文。本基准仍按【全量引擎】测量，即
// 「单条消息渲染成本的上界」，用于评估历史页同步渲染的绝对代价，
// 故保持不变；懒装配削减的是无需该能力的会话的加载量，而非单次
// 渲染耗时。
// ============================================================
