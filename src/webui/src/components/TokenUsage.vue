 ...
 *  `sid~agent` 旧键泄漏出的独立会话 UUID（8/20 前的会话键形态）；
 *  ②已更名/下线的遗留预设 id（__minimal__ / __blank__ 等，不在当前预设
...
function renderCloud() {
  ...
  // ---- user / self 与群聊流量：chordPairs 已在过滤口径里整体排除
  //     （self 恒排除；user 视 includeUserSelf；群聊/预设/名册外恒排除），
  //     分组与弧段不再需要按 by_agent 总量做扣减 ----

  // user 伪节点（includeUserSelf 开启且 by_agent 无 user 行时合成）：
  // by_pair 的 user↔agent 弦需要 user 作为图节点；缺失时弦端会错挂到
  ...
  // 的运行模式、非协作端点；②名册外端点（遗留预设 id / 迁移期泄漏的会话
  // UUID）——均从弧段、分组与「其他」归并中整体剔除
  const usableRows = data.value.by_agent.filter(a => !isPresetCp(a.agent) && isKnownCp(a.agent));
  const agentRows: AgentUsage[] =
    includeUserSelf.value && userPairFlow > 0 && !usableRows.some(a => a.agent === 'user')
      ? [...usableRows, {
          agent: 'user',
          total_prompt_tokens: 0,
          total_completion_tokens: 0,
          total_tokens: userPairFlow,
          total_react_steps: 0,
          total_cache_hit: 0,
          total_cache_miss: 0,
  ...
  // ---- 分组口径 = 弦图自己的度量（协作流量）：端点参与的全部弦流量 ----
  // 旧口径按 by_agent 总用量选 main、弧长却按协作流量分配——"总量小但
  // 协作多"的 Agent（editor/chat_agent 等）被挤进「其他」，其他弧虚胖；
  // 现与弧长/弦宽同口径，占比与视觉一致。
  const flowByAgent = new Map<string, number>();
  for (const p of chordPairs.value) {
    flowByAgent.set(p.a, (flowByAgent.get(p.a) ?? 0) + p.total_tokens);
    flowByAgent.set(p.b, (flowByAgent.get(p.b) ?? 0) + p.total_tokens);
  }
  const flowOf = (ag: string) => flowByAgent.get(ag) ?? 0;
  const flowTotalForGroup = chordPairs.value.reduce((s, p) => s + p.total_tokens, 0) || 1;

  // ---- 按协作流量占比分组：占比 < 阈值 → 「其他」（保底 top 5）----
  const THRESHOLD_PCT = 2;
  const sortedByFlow = [...agentRows].sort((a, b) => flowOf(b.agent) - flowOf(a.agent));
  let main = agentRows.filter(a => (flowOf(a.agent) / flowTotalForGroup) * 100 >= THRESHOLD_PCT);
  if (main.length < 5) main = sortedByFlow.slice(0, 5); // 保底 top 5（按协作流量）
  const mainSet = new Set(main.map(a => a.agent));
  // 「其他」= 有协作流量但未进 main 的 Agent（零流量 Agent 不占「其他」名分）
  const otherAgents = agentRows.filter(a => !mainSet.has(a.agent) && flowOf(a.agent) > 0);

  // ---- 节点：main 按 agent_id 排序位置稳定，其他放末尾 ----
  const nodes: Array<{ agent: string; tokens: number; isOther: boolean }> = [];
  for (const a of [...agentRows].filter(x => mainSet.has(x.agent)).sort((x, y) => x.agent.localeCompare(y.agent))) {
    nodes.push({ agent: a.agent, tokens: flowOf(a.agent), isOther: false });
  }
  if (otherAgents.length > 0) nodes.push({ agent: 'other', tokens: otherAgents.reduce((s, a) => s + flowOf(a.agent), 0), isOther: true });
  ...
  // tooltip 百分比分母 = 协作流总量（与分组阈值同口径，Σ弦=100%）。注意不能
  // 用 rowTotal：它是弧段几何分母，每条弦在两端各计一次（≈2× 协作流），
  // tooltip 数字会砍半、与 2% 分组规则对不上（如 chat_agent 3.8% 显示成 1.9%）
  chordFlowTotal = flowTotalForGroup;
  nodes.forEach((nd, i) => {
    nd.tokens = rowSum[i];
    const m = arcMetas.get(nd.agent);
    if (m) { m.tokens = rowSum[i]; m.pct = (rowSum[i] / flowTotalForGroup) * 100; }
  });
  ...
  for (const a of [...main].sort((x, y) => flowOf(y.agent) - flowOf(x.agent)).slice(0, 10)) {
