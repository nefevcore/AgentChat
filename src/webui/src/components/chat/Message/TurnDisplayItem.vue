...
/** final 的流式态：无思维链的纯文本轮由 final 承载正文流式（走分块渲染
 *  路径 + typing indicator；此前恒 false → 全帧全量 markdown 渲染 O(n²)） */
const finalIsStreaming = computed(() => !hasChain.value && isStreaming.value);
  ...
  <div class="turn-item" :class="isSelf ? 'turn-right' : 'turn-left'">
    ...
    <template v-if="!hasChain && finalMsg">
      ...
      <div v-else class="turn-bubble turn-bubble-left">
        <AssistantMessage
          :message="finalMsg" :index="index" :is-streaming="finalIsStreaming"
          :sender-avatar="senderAvatar" :sender-name="senderName"
    ...
    <template v-if="hasChain">
      ...
      <div v-show="isExpanded" class="chain-body">
        <template v-for="(step, sIdx) in meaningfulSteps" :key="stepKey(step, sIdx)">
          <AssistantMessage
