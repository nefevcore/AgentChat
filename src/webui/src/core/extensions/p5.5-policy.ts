...
/** 单选择器安全校验（允许普通选择器；禁分号/花括号/反斜杠——反斜杠可
 *  在 CSS 解析期反转义出 url( 等被黑名单拦截的形态，必须直接禁掉） */
function assertSelector(selector: string): void {
  if (!selector || selector.includes(';') || selector.includes('{') || selector.includes('}') || selector.includes('\\')) {
    throw new Error(`非法选择器: ${selector}`);
  }
}

/** 声明值安全校验：禁反斜杠（CSS 转义逃逸面，如 `\75 rl(` 解析为 `url(`） */
function assertDeclarationBody(body: string): void {
  if (body.includes('\\')) {
    throw new Error('global-style 声明值不允许反斜杠转义');
  }
}

/** :root 块 body 逐 declaration 校验：只允许 `--*` 自定义属性（此前 body
 *  完全不校验，可写 font-family/color-scheme/filter 等任意全局属性，
 *  绕开"每个选择器强制前缀"的隔离承诺） */
function assertRootBody(body: string): void {
  assertDeclarationBody(body);
  for (const decl of body.split(';')) {
    const trimmed = decl.trim();
    if (!trimmed) continue;
    const colon = trimmed.indexOf(':');
    const prop = colon >= 0 ? trimmed.slice(0, colon).trim() : trimmed;
    if (!prop.startsWith('--')) {
      throw new Error(`:root 块只允许 CSS 自定义属性（--var），不允许: ${prop}`);
    }
  }
}
