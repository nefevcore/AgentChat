// ============================================================
// src/plugins/builtin/tools/session.ts —— 会话工具（query_history/inspect_session/continue_turn）
//
// 迁移自旧 mod 的 tools/{query_history,inspect_session,continue_turn}，按领域聚合。
//
// 适配新架构：
//   · 路径：旧 getGlobalConfig().sessionsDir → workspaceRoot()/sessions
//   · registry：旧 getAppState().registry → PluginServices.router.getRegistry()
//   · router：旧 getAppState().router → PluginServices.router
//   · 会话文件：旧 <lo>/<hi>/messages.jsonl（Canonical 排序）→ 新 <dialogId>/messages.jsonl
//     （dialogId 方向敏感：<from>__<to>，与 hooks/session saveSession 一致）
//   · token 工具：旧 @utils/tokens → 本层 shared
//
// 依赖方向：仅依赖本层 shared + @agents/config + @core/types + define-tool + 本层 types。
// ============================================================
...
import { workspaceRoot, safeTruncate } from './shared';
import { chatSessionFile, groupSessionFile } from '../paths';

// ============================================================
// 共享：会话文件路径（与 hooks/session 一致）
// ============================================================

/** 1:1 会话文件：<ws>/sessions/chat~<lo>~<hi>/messages.jsonl */
...
export function makeQueryHistoryTool(config: AgentConfig, services: PluginServices): Tool {
  ...
  return defineTool({
    ...
    execute: async (args) => {
      ...
      try {
        ...
        // 倒序 → 分页（按消息条数；旧实现按 user 链，新存储平铺，取最近 N 条）
        messages.reverse();
        messages = messages.slice(offset, offset + limit);
        if (keyword) {
          const kw = keyword.toLowerCase();
...
export function makeContinueTurnTool(config: AgentConfig, services: PluginServices): Tool {
  ...
  return defineTool({
    ...
    execute: async (args) => {
      ...
      if (!router) {
        return JSON.stringify({ status: 'error', data: { message: 'AgentRouter 未注入 PluginServices' } });
      }
      try {
...
/** 会话类工具工厂 */
export function makeSessionTools(config: AgentConfig, services: PluginServices): Tool[] {
  return [
