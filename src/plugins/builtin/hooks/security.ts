...
import type { ToolExecutionStartHook } from '@core/context';
import type { ToolExecutionStartResult } from '@core/context';
...
function resolveCallerTags(agentId: string, agentsDir: string): string[] {
  try {
    if (!agentsDir || !require('fs').existsSync(agentsDir)) return [];
    const fs = require('fs');
    for (const entry of fs.readdirSync(agentsDir, { withFileTypes: true })) {
      if (!entry.isDirectory()) continue;
      const cfgPath = path.join(agentsDir, entry.name, 'config.json');
      if (!require('fs').existsSync(cfgPath)) continue;
  ...
  return async (toolName: string, args: Record<string, any>): Promise<ToolExecutionStartResult> => {
    ...
    if (PROFILE_TOOLS.has(toolName)) {
      ...
      if (toolName === 'update_agent_profile') {
        ...
        if (fields) {
          // 新架构（2026-08-07）：旧 tools/pre_hooks/post_hooks 统一由 plugins（对象数组）声明，
          // tags 仍为字符串数组（能力标签）
