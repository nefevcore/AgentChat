...
export const mathTools: Tool[] = [
  defineTool({
    ...
    description: '计算数学表达式。支持 + - * / % ** ( )，以及常用函数（无需 Math. 前缀）：sqrt/pow/abs/floor/ceil/round/min/max/sin/cos/tan/log/log2/log10/exp/trunc/cbrt/hypot，常量 PI/E。示例：1+2*3 → 7；sqrt(16) → 4；2**10 → 1024；sin(PI/2) → 1。',
    parameters: {
      type: 'object',
      properties: {
