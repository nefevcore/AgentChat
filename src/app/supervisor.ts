...
//
// 职责：
//   1. spawn 工作进程（src/app/index.ts 作为主入口）
//   2. 监听退出码：
//       42  = 主动重启 → 重新 spawn
...
//
// 用法：
//   npx tsx src/app/supervisor.ts [--port=3830] [--workspace=...]
//   或编译后：node dist/src/app/supervisor.js
// ============================================================
...
function resolveEntry(): string {
  ...
  // 开发模式：src/app/index.ts（tsx 直跑，路径别名由 tsx 处理）
  const srcEntry = path.join(ROOT, 'src', 'app', 'index.ts');
