// ============================================================
// @agentchat/durable-interaction/src/types.ts —— 通用持久化交互契约
//
// 领域无关：不 import AgentChat 任何类型。
// 语义（对应 durable suspension / resumable interaction 模式）：
...
export interface DurableInteractionInput {
  /** 路由/会话身份（业务方自己定义，如 convKey / orderId / workflowId） */
  key: string;
  /** 交互种类（如 ask_questions / approval / webhook；恢复方据此选择处理方式） */
  kind: string;
  /** 展示/处理所需的完整数据（问题、选项、回调参数等） */
  payload: JsonValue;
  /** 与执行点关联的稳定关联键（如 tool_call_id / stepId）；恢复对账用 */
