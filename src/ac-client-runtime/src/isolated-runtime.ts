// ============================================================
// ac-client-runtime/src/isolated-runtime.ts —— iframe isolated 档运行时
//（M27 S4/D21：自 webui/src/isolated-runtime.ts 原样迁入——扩展宿主基建；
// webui 侧入口 shim 维持 ui-plugin-iframe.html 的旧引用路径）
//
...
//
// DOM 面经结构化类型访问（本包进根 tsc 程序，不引 DOM lib——
// lastContext.ts 同款纪律）。
// ============================================================
...
/** 结构化 DOM 面（iframe 档零 DOM lib：window/postMessage/message 事件） */
...
interface WindowLike {
  ...
  parent: { postMessage(message: object, targetOrigin: string): void };
  addEventListener(type: 'message', listener: (event: MessageEventLike) => void): void;
}
interface DocumentLike {
  body: { textContent: string };
}
/** fetch init 的结构化形（Headers 实例/数组对/普通对象三态兼容） */
interface StructHeadersLike {
  forEach(callback: (value: string, key: string) => void): void;
}
interface StructRequestInit {
  method?: string;
  headers?: Record<string, string> | Array<[string, string]> | StructHeadersLike;
  body?: string | null;
}
...
async function request<T = unknown>(path: string, init: StructRequestInit = {}): Promise<T> {
  const id = ++requestSeq;
  ...
  if (init.headers) {
    ...
    } else if (typeof (init.headers as StructHeadersLike).forEach === 'function') {
      (init.headers as StructHeadersLike).forEach((value, key) => { headers[key] = value; });
    } else {
...
interface IsolatedUiExtensionContext {
  name: string;
  request<T = unknown>(path: string, init?: StructRequestInit): Promise<T>;
...
// 本文件是模块（非脚本）——declare const 作用域不外溢整个程序
export {};
