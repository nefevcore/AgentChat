...
export interface ConfigRouterDeps {
  /** 可选：保存全局配置后热重载所有已注册 Agent（新 LLM 池立即生效） */
...
export function createConfigRouter(deps: ConfigRouterDeps = {}): Router {
  ...
  router.post('/', (req: Request, res: Response) => {
    try {
      ...
      const reloaded = configService.reloadGlobalConfig();

      // 保存后立即热重载已注册 Agent：AgentLoader 捕获的 globalConfig 已被
      // 原地更新，重读磁盘差异配置即可解析到新 LLM 池（无需重启）。
      if (reloaded && deps.agentService) {
        for (const agent of deps.agentService.listBasic()) {
          if (agent.virtual) continue;
          try {
            deps.agentService.hotReloadAgent(agent.id, path.join(reloaded.agentsDir, agent.id));
          } catch (err: any) {
            logger.warn(`全局配置保存后热重载 Agent "${agent.id}" 失败: ${err?.message ?? String(err)}`);
          }
        }
      }

      logger.info(`[Config API] 全局配置已保存并热重载（已注册 Agent 立即生效）`);
