...
import { setGlobalConfig } from '../src/runtime';
...
describe('AgentService', () => {
  ...
  it('saveAgentConfig：普通保存不携带装配字段时保留磁盘现值（PUT assembly 分权）', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'svc-assembly-'));
    try {
      fs.mkdirSync(path.join(tmp, 'agentA'), { recursive: true });
      const configPath = path.join(tmp, 'agentA', 'config.json');
      fs.writeFileSync(configPath, JSON.stringify({
        agent_id: 'agentA', name: 'AgentA',
        presets: ['agentchat-math'], tools: ['math'], hooks: { runStart: ['h1'] },
      }), 'utf-8');

      const svc = new AgentService({ registry: new AgentRegistry() });
      svc.saveAgentConfig('agentA', path.join(tmp, 'agentA'), { name: 'AgentA2', tags: ['admin'] });

      const saved = JSON.parse(fs.readFileSync(configPath, 'utf-8'));
      expect(saved.name).toBe('AgentA2');
      expect(saved.presets).toEqual(['agentchat-math']);
      expect(saved.tools).toEqual(['math']);
      expect(saved.hooks.runStart).toEqual(['h1']);
    } finally {
      fs.rmSync(tmp, { recursive: true, force: true });
    }
  });
