...
export const tool: Tool = {
  definition: {
    ...
    function: {
      ...
      description:
        '向群聊房间发送消息。' +
        '此工具投递后立即返回，不等待其他 Agent 的回复。',
      parameters: {
        type: 'object',
        properties: {
          room_id: {
            type: 'string',
            description: '目标房间 ID（可通过 list_rooms 获取可用清单）',
          },
          message: {
            type: 'string',
            description: '要发送到房间的消息内容',
          },
        },
  ...
  execute: async (args: Record<string, any>) => {
    ...
    try {
      ...
      return `消息已投递到房间 "${room.name}" (${room_id})，` +
        `送达 ${result.delivered_to.length} 个参与者：${result.delivered_to.join(', ') || '(无)'}`;
    } catch (err: any) {
