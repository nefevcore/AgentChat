// ============================================================
// list_rooms 工具 —— 获取群聊房间清单
//
// 设计原则：
//   1. 无参数调用，返回所有房间信息
//   2. 标注当前 Agent 参与的/未参与的房间
...
export const tool: Tool = {
  definition: {
    ...
    function: {
      name: 'list_rooms',
      description:
        '列出所有群聊房间，含 ID、名称、参与者，并标注当前 Agent 是否在其中。',
      parameters: {
        type: 'object',
        properties: {},
        required: [],
  ...
  execute: async (args: Record<string, any>) => {
    ...
    if (!router) {
      return `[list_rooms] 错误：AgentRouter 未注册到 AppState。`;
    }
    const r = router as AgentRouter;

    const roomManager = r.getRoomManager();
    if (!roomManager) {
      return `[list_rooms] 错误：RoomManager 未初始化。`;
    }
    ...
    let result = `共 ${allRooms.length} 个房间（你参与了 ${myRooms.length} 个）：\n\n`;
