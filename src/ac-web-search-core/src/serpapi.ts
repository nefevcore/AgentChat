// ============================================================
// ac-web-search-core/src/serpapi.ts —— SerpAPI Provider（src 原样平移）
// ============================================================
...
export function createSerpApiProvider(): SearchProvider {
  return {
    ...
    validateConfig(cfg: ProviderConfig): void {
      if (!cfg.apiKey) {
        throw new Error('未配置 SerpAPI 密钥。可前往 https://serpapi.com 免费注册（每月 100 次免费搜索）。');
      }
    },
    ...
    async search(params: SearchParams, cfg: ProviderConfig): Promise<SearchResponse> {
      ...
      const answer = data.answer_box?.answer ?? data.answer_box?.snippet ?? data.knowledge_graph?.description ?? null;
