// ============================================================
// ac-agents/tests/agents-row.test.ts —— 行包双半边宿主面验收
...
describe('S3-1b · ac-agents 宿主半边（boot graph 声明）', () => {
  it('行装载 → boot graph 含 agents；卸载 → 级联回缩 + 变更帧', async () => {
    ...
    expect(ctx.webui.listBootGraph().map((g) => g.name)).toContain('agents');
    await fiber.dispose();
    expect(ctx.webui.listBootGraph().map((g) => g.name)).not.toContain('agents');
    expect(changed.filter((n) => n === 'agents').length).toBeGreaterThanOrEqual(2);
    off();
  });
});
