...
**先排除一个猜测**：不是"只有一条用户消息时不传工具"。`agent.ts` 的 `executeLoop` 每个 ReAct step 无条件从
...
一旦某 step Agent 空转（不调工具 → 不写 `messages.jsonl`），下一步上下文里 **assistant 更稀疏、
