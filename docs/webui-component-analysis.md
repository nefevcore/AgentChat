...
└── src/
    ...
    └── components/
        ...
        └── chat/
            ...
            │   ├── TurnDisplayItem.vue     # ★ 核心：统一对话轮次渲染
            │   ├── UserMessage.vue
...
- `chat.start` / `chat.turn.start` / `chat.turn.end` / `chat.end` / `chat.interrupted` / `chat.interaction`
...
> 这些遗留说明：前端经历过"Message 路由组件 → TurnDisplayItem 统一轮次"和"群组独立列表 → Agent+群组统一列表"两次演进，旧组件未清理。
...
3. **Turn 数据模型**：后端流式事件被增量构建成 `Turn{steps[], final}` 结构，前端按"轮次"渲染，配合时间分隔符与 trigger 分隔符平铺。
