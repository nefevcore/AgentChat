...
| 字段名 | 标签 | 描述 | 类型 | 默认值 |
|--------|------|------|------|--------|
| `maxContextTokens` | `最大上下文 Token` | `上下文压缩触发阈值（估算 token 数）` | number | 100000 |
| `keepRecentRatio` | `保留最近比例` | `压缩时保留的最近消息比例` | number | 0.10 |
| `summaryPreviewLen` | `摘要预览长度` | `压缩摘要中每条消息的预览截断长度` | number | 200 |
| `idleArchiveSec` | `空闲归档秒数` | `无对话自动归档的等待时间` | number | 1800 |
...
| 字段名 | 标签 | 描述 | 类型 | 默认值 |
|--------|------|------|------|--------|
| `tools` | `工具列表` | `启用工具定义列表装配` | checkbox | true |
| `guidelines` | `使用指南` | `启用动态指引装配` | checkbox | true |
| `windowsEnv` | `Windows 环境` | `启用 Windows 环境信息注入` | checkbox | true |
| `skills` | `技能清单` | `启用技能清单装配` | checkbox | true |
| `datetime` | `日期时间` | `启用当前日期时间注入` | checkbox | true |
| `mcp` | `MCP 发现` | `启用 MCP 工具和资源发现` | checkbox | true |
...
| **name** | `bash` |
| **label** | `Bash 终端` |
| **definition.description** | `执行 Shell 命令。有专用工具时优先使用专用工具。` |
| **ns** | `tool.bash` |
...
| **definition.description** | `精确替换文件中的文本。` |
| **ns** | `tool.edit` |
...
| **definition.description** | `获取所有可用 Agent 的清单。返回每个 Agent 的 ID、名称和类型（虚拟/真实）。用于了解当前有哪些 Agent 可以与之通讯。` |
| **ns** | `tool.list_agents` |
...
| **definition.description** | `计算数学表达式，支持基本运算（+、-、*、/、^）、三角函数（sin、cos、tan）、对数（log、ln）、平方根等常用数学函数` |
| **ns** | `tool.math` |
...
| **name** | `read` |
| **label** | `读取文件` |
| **definition.description** | `读取文件内容或列出目录结构。` |
| **ns** | `tool.read` |
...
| **definition.description** | `向另一个 Agent 发送消息并获取其响应。这是 Agent 间通讯的唯一方式。无状态设计：每次调用独立，不会自动传递上下文。如需多轮对话，请在前一条响应的基础上自行构造后续消息。` |
| **ns** | `tool.send_agent` |
...
| **作用** | 自动注入调用方 agentId 到 send_agent 的 args.from，LLM 无法伪造身份。 |
