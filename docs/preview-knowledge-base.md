...
- **钩子注册**：`ctx.hooks.register(kind, name, factory)`（kind: runStart/runEnd/turnStart/turnEnd/toolExecutionStart/toolExecutionEnd/fallback）；钩子名规范为 `<插件名>.<钩子>`（如 `agent-prompt.build-system-prompt`、`hooks.log-tool`）。
