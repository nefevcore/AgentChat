...
> 版本：v0.6.3（2026-08-16 随 Router 重构更新触发语义） · 机制自 v0.5.x 起实测沿用
...
- 共享 messages.jsonl 使得双方整理 run loadHistory 都能读到**完整文件**（尚未归档），天然解决"发送方无法自然分析"

```
触发 → 写标记 → 双方/多参与者整理 run（完整上下文）→ 全部完成 → 归档
```

## 实现位置（v0.6.3 插件化架构）

- **核心编排**：`src/svc/archive/src/index.ts`（ArchiveService；runEnd 超阈值 / 手工归档 / 批量 / 超时扫描 / 空闲归档统一入口），插件行 `src/svc/archive/src/plugin.ts` 提供 `ctx.archive` 并写回 `services.archiveSession / idleReset`
- **整理 run标志**：`CurrentContext.meta` 通用扩展通道（`src/core/agent-loop/src/context.ts`，L1 不解释键）+ `TriggerOptions.meta`（`src/agents/router/src/router.ts`）
- **常量键**：`META_ARCHIVE_REVIEW = 'archive-review'`（`src/tools/tools` 经 `@agentchat/toolkit` re-export）
- **runEnd 钩子**：`makeArchiveSessionHook`（`src/agent-session/agent-session/src/run.ts`）经 `PluginServices.archiveSession` 委托 `ArchiveService.handleRunEnd`
- **会话持久化跳过**：`saveSession`（`src/agent-session/agent-session/src/session.ts`）检测 `ctx.meta?.[META_ARCHIVE_REVIEW]` → 整理 run 不落盘
- **装配注入**：`src/boot/boot/src/plugin-finalize.ts` 启动归档超时 watcher；`src/host/server/src/service-plugin.ts` 把 archive 回调注入 HistoryService
...
① B runEnd 检测超阈值（actualTotal 或 estimatedTotal > maxContextTokens × archiveTokenRatio）
   ...
   → trigger 双方整理 run（`void router.trigger` fire-and-forget；meta['archive-review']=true, target=对方；
     带 meta 属 run 级选项，会话繁忙时默认 next-run：后台等待空闲后新开 run）

② 整理 run（特殊 meta['archive-review']）：
...
`src/svc/archive/tests/archive.test.ts`（归档编排用例）：
- requestArchive：写 pending + 触发双方整理 run / 幂等 / 虚拟 counterpart 单侧 ✅
- archiveAllActiveSessions：批量触发（跳过空/自对话/群聊/已有 pending）✅
- scanPendingArchives：超时强制归档 / 未超时清理 + 审查标记 ✅
- handleRunEnd：整理 run全完成 → 归档 + 清理标记 / 未全完成仅写 done / 超阈值触发 / 未超阈值跳过 ✅
- archiveAndRebuild 二次归档去重（跳过上次归档最后一条）✅

已知差异（相对旧实现）：
- `getPendingMessages/clearPendingMessages`（WeakMap 本次缓存）不再需要——新架构 runEnd 的
  `saveSession` 已把整次 run 消息落盘，归档时 `ctx.history` 即完整消息源
